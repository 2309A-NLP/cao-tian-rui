...
LLM回：调 book_appoint![img](file:///C:\Users\ctr\AppData\Local\Temp\ksohtml1700\wps1.jpg)
...
and 查过号源![img](file:///C:\Users\ctr\AppData\Local\Temp\ksohtml1700\wps2.jpg)但没挂号![img](file:///C:\Users\ctr\AppData\Local\Temp\ksohtml1700\wps3.jpg):
...
时间处理的完整链路（呼应你的问题）用户说"明天下午2点"↓utils.parse_time() 解析（用 date.today() 本地时间）↓_pre_validate() 校验日期范围是否在[今天, 今天+14天]↓通过 → agent_prompts.build_system_prompt() 把今天日期注入System Prompt↓LLM看到"今天=2026-07-06"这个明确信息，才能正确推算"明天=2026-07-07"System Prompt里的日期是每次请求时动态生成的（![img](file:///C:\Users\ctr\AppData\Local\Temp\ksohtml1700\wps4.jpg)(F:\kimi project\医疗agent1\01-挂号管理\backend\src\agent_prompts.py#L30) 不是常量），目的是防止LLM用它训练时的日期知识乱推算。
