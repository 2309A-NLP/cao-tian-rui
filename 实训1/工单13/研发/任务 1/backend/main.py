...
def _init_components(config: AppConfig, need_rag: bool = True):
    """统一初始化 LLM、VectorStore 和 RAGEngine。
    
    Args:
        config: 应用配置
        need_rag: 是否需要创建 RAGEngine（索引等场景不需要）
    Returns:
        (llm, vec, rag) 三元组
    """
    ...
    print(f"[config] embedding_batch_size={config.embedding_batch_size}")
    if not vec.load_model() or not vec.connect_milvus():
        print("错误：向量存储初始化失败")
        return llm, vec, None
...
def index_pdf(pdf_path: str, config: AppConfig, vector_store: VectorStore,
    ...
    use_hybrid: 是否使用混合策略处理器（Qwen3-VL + 页面分类）
    """
    ...
    # 解析 PDF
    if use_hybrid and config.hybrid_processor_enabled:
        # 使用混合策略处理器
        print("🔄 使用混合策略处理器（Qwen3-VL + 页面分类）")
        processor = HybridPDFProcessor(
            pdf_path=pdf_path,
            output_dir=config.output_dir,
            qwen3_api_key=config.qwen3_api_key,
            qwen3_base_url=config.qwen3_base_url,
        ...
        # 处理所有页面
        chunks = processor.process(resume=resume)
        
        # 保存处理统计
        stats = processor.stats
        print(f"生成了 {len(chunks)} 个文档块")
        print(f"统计：{stats['total']} 页, "
              f"纯文本:{stats['pure_text']} 表格:{stats['table']} "
              f"图片:{stats['image']} 混合:{stats['mixed']} "
              f"错误:{stats['errors']} 耗时 {stats['total_time']:.1f}s")
        
        # 转换为 index_documents 支持的格式并存入 Milvus
        print("正在向量化并存入 Milvus...")
        index_chunks = processor.to_index_format(chunks, filename=filename)
        # 按章节合并相邻页，再智能分块，提高检索精度
        chunk_size = getattr(config, "chunk_size", 1200)
        chunk_overlap = getattr(config, "chunk_overlap", 150)
        index_chunks = HybridPDFProcessor.merge_by_section(
            index_chunks, max_chars=chunk_size, overlap=chunk_overlap
        )
        print(f"按章节合并分块后: {len(index_chunks)} 个文档块 (max_chars={chunk_size})")
        if not vector_store.index_documents(index_chunks, filename=filename):
            print("错误：文档索引失败")
            return 0
...
def run_interactive(config: AppConfig):
    ...
    # 初始化全部组件
    llm, vec, rag = _init_components(config)
    if rag is None:
        print("错误：组件初始化失败")
        return
...
def main():
    ...
    parser.add_argument("--config", type=str,
                        default="",
                        help="指定配置文件路径（默认自动检测：先找 backend/config.json，再找项目根目录 config.json）")
    parser.add_argument("--show-config", action="store_true", help="显示配置并退出")
    parser.add_argument("--hybrid", action="store_true", 
                        help="使用混合策略处理器（需要配置 Qwen3-VL API Key）")
    parser.add_argument("--api-key", type=str, help="Qwen3-VL API Key（覆盖配置文件）")
    parser.add_argument("--api-secret", type=str, help="（已废弃，仅保留兼容）")
    parser.add_argument("--resume", action="store_true",
                        help="从 checkpoint 恢复处理（跳过已完成的页面，用于崩溃续跑）")
    args = parser.parse_args()

    # 加载配置（自动检测 config.json 位置）
    config_path = args.config
    if not config_path:
        # 1. 先看 backend/config.json
        local_config = os.path.join(os.path.dirname(__file__), "config.json")
        # 2. 再看项目根目录 config.json
        parent_config = os.path.join(os.path.dirname(os.path.dirname(__file__)), "config.json")
        if os.path.exists(local_config):
            config_path = local_config
        elif os.path.exists(parent_config):
            config_path = parent_config
        else:
            config_path = local_config  # fallback，让 AppConfig 处理不存在的情况
    config = AppConfig.load(config_path)
    
    # 如果命令行启用 hybrid，强制覆盖配置
    if args.hybrid:
        config.hybrid_processor_enabled = True
    
    # 覆盖 API 密钥（如果通过命令行提供）
    if args.api_key:
        config.qwen3_api_key = args.api_key
    
    # 如果启用混合模式但未配置API密钥，给出警告
    if args.hybrid and not config.qwen3_api_key:
        print("⚠️ 警告：混合模式需要 Qwen3-VL API Key")
        print("请配置以下方式之一：")
        print("1. 在 config.json 中设置 qwen3_api_key")
        print("2. 通过命令行参数提供：--api-key <key>")
        return
    ...
    if args.scan is not None:
        ...
        _, vec, _ = _init_components(config, need_rag=False)
        total_chunks = 0
        skipped = 0
        for pdf_file in pdf_files:
            ...
            if chunks == 0:
                skipped += 1
            total_chunks += chunks
        mode_text = "混合策略" if args.hybrid else "标准策略"
        print(f"\n扫描完成！使用{mode_text}共索引 {total_chunks} 个文档块，跳过 {skipped} 个已存在的文件")
        return
    ...
    if args.reindex and not args.index:
        ...
        for pdf_file in pdf_files:
            ...
            chunks = index_pdf(pdf_path, config, vec,
                              drop_existing=False, force=True, use_hybrid=args.hybrid,
                              resume=args.resume)
            total_chunks += chunks
        ...
        if args.hybrid:
            try:
                ...
                # 收集所有文件的 hybrid chunks
                hybrid_chunks_output = os.path.join(output_dir, "hybrid_vl_output", "_all_hybrid_chunks.json")
                print(f"\\n💾 Hybrid 中间产物已保存在: {os.path.dirname(hybrid_chunks_output)}/")
                print(f"   - page_images/  : 每页 PNG 截图")
                print(f"   - vl_results/   : VL 返回的 markdown/html/json")
                print(f"   - _manifest.json: 处理记录")
                print("您可以在 Windows 资源管理器中直接打开查看效果")
            except Exception as e:
                print(f"  保存中间产物信息时出错: {e}")

        print(f"\\n最终 Milvus 向量总数: {vec.count()}")
        return
