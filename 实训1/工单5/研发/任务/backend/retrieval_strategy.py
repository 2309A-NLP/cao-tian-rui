...
class RetrievalExecutor:
    ...
    def search_with_rerank(self, query: str, config: RetrievalConfig) -> list[dict]:
        ...
        # 1b. 强制召回：从索引中 grep 含关键短语的 chunk 注入候选池
        sources = self._force_recall(query, sources)

        # 1c. 公司路由过滤：识别查询目标公司，排除对手公司chunk
        try:
        ...
        if config.rerank_method == "reranker":
            ...
            sources = self._keyword_rerank(query, sources)  # 再做一次更细致的关键词排序
        elif config.rerank_method == "adaptive":
            sources = self._adaptive_rerank(query, sources)
        # "none" 不做额外重排
    ...
    def _search_hybrid(self, query: str, config: RetrievalConfig) -> list[dict]:
        ...
        has_hybrid = (
            ...
            and getattr(self.vector_store, 'bm25_ready', False)
        )
        if has_hybrid:
            # 使用 vector_store 自带的 hybrid_search（RRF 融合）
            all_sources = []
            seen_keys = set()
            query_variants = [query]
            if config.query_rewrite:
            ...
            for vq in query_variants:
                ...
                for s in hits:
                    ...
                    if key not in seen_keys:
                        seen_keys.add(key)
                        all_sources.append(s)
            # hybrid 模式下 threshold 对原始向量分数过滤，不是对 RRF 分数
            if config.similarity_threshold > 0:
                filtered = []
                for s in all_sources:
                    vs = s.get("vector_score", s.get("score", 0))
                    if vs >= config.similarity_threshold:
                        filtered.append(s)
                sources = filtered
            else:
                sources = all_sources
            ...
            logger.debug(f"混合检索(fallback): query='{query[:30]}', 命中={len(sources)} 条")
            return sources
    ...
    def _keyword_rerank(query: str, sources: list[dict],
        ...
        for phrase_len in range(8, 2, -1):
            for i in range(len(clean_query) - phrase_len + 1):
                ...
                if len(phrase) >= 3 and phrase not in stop_words:
                    # 只保留有意义的短语（不全是停用词）
                    if any(w not in stop_words for w in re.findall(r'[\u4e00-\u9fff]{2}', phrase)):
                        key_phrases.add(phrase)
        key_phrases = sorted(key_phrases, key=len, reverse=True)  # 长短语优先

        # 查询特定关键词（排除公司名等通用词，用于识别查询意图）
        company_names = {'武汉兴图新科', '兴图新科', '武汉力源', '力源信息',
                         '电子股份有限', '股份有限', '招股说明书', '申报稿',
                         '招股意向书', '兴图', '新科'}
        query_intent_words = [w for w in keywords
                              if w not in company_names and len(w) >= 2]
        ...
        for s in sources:
            ...
            for phrase in key_phrases:
                if phrase in text:
                    boost += 0.5 if len(phrase) >= 4 else 0.3
            ...
            elif intent_hits >= 1 and len(query_intent_words) >= 2:
                # 查询有多个意图词但只命中1个 → 弱相关，小惩罚
                boost -= 0.1
            ...
            # 8. 查询意图关键词完全未命中 → 大幅惩罚
            if intent_hits == 0 and query_intent_words:
                boost -= 1.0
    ...
    def _force_recall(self, query: str, sources: list[dict],
        ...
        # 排除通用词后提取有意义的关键词
        company_names = {'武汉兴图新科', '兴图新科', '武汉力源', '力源信息',
                         '电子股份有限公司', '股份有限公司', '有限公司',
                         '招股说明书', '申报稿', '招股意向书'}
        stop_words = {'公司', '股份', '有限', '电子', '的', '是', '在', '有',
                      '多少', '哪些', '分别', '各', '以及', '或者', '什么'}
        ...
        for w in query_words:
            if w in stop_words or w in company_names:
                continue
            if any(cn in w or w in cn for cn in company_names):
                continue
            if len(w) >= 2:
                recall_phrases.append(w)

        # 如果关键词太少，也从完整查询中提取 3-5 字子串
        if len(recall_phrases) < 2:
            for phrase_len in range(5, 2, -1):
                for i in range(len(clean_query) - phrase_len + 1):
                    phrase = clean_query[i:i + phrase_len]
                    if phrase in stop_words or phrase in company_names:
                        continue
                    if any(cn in phrase or phrase in cn for cn in company_names):
                        continue
