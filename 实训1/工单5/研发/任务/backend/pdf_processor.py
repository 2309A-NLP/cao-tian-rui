...
class PDFProcessor:
    ...
    def get_chunks(self, chunk_size: int = DEFAULT_CHUNK_SIZE,
        ...
        """
        将文本分块，包含元数据
        v3: 支持字段类型检测 (title/abstract/table/body)
        ...
        # 保存分块到 output（按文件名区分，避免多个PDF互相覆盖）
        ...
        chunk_file = chunk_dir / f"chunks_{_pdf_name}.json"
        chunk_data = [{"chunk_id": c["chunk_id"], "page": c["page"],
                        "text": c["text"], "text_length": len(c["text"])} for c in chunks]
        with open(chunk_file, "w", encoding="utf-8") as f:
            json.dump(chunk_data, f, ensure_ascii=False, indent=2)

        # 同时写一个汇总文件（所有PDF的chunks合并）
        all_file = chunk_dir / "all_chunks.json"
        all_chunks = []
        for cf in sorted(chunk_dir.glob("chunks_*.json")):
            if cf.name == "all_chunks.json":
                continue
            try:
        ...
        logger.info(f"文本分块完成: {len(chunks)} 个块（累计 {len(all_chunks)} 个）")
        return chunks
