...
def compute_simple_metrics(questions, rag_answers, lightrag_answers) -> dict:
    ...
    def _score_answer(answer: str, question: str) -> dict:
        ...
        # 判断是否真的出错了（API/系统错误），而非"没找到数据"
        is_error = "失败" in answer and "查询失败" in answer or "Traceback" in answer or "Error" in answer[:100]

        has_answer = 0 if is_error else 1
        has_number = 1 if re.search(r'[\d,.]+[%万元个条亿]', answer) else 0
